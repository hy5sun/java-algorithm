...
mid 보다 작으면 해당 좌표에는 말을 두지 않고, 다음 좌표로 넘어간다.
그렇게 끝까지 갔을 때 설치한 말이 C와 일치하는지 확인하고, 작으면 최소 거리가 더 짧아야 된다는 것이고, 최소한 mid보다는 작다는 것이므로 rt = mid - 1 처리를 한다.
만약 C보다 많거나 같다면 현재 mid 보다 더 큰 값이 최댓값이 될 수 있다는 것이므로 lt = mid + 1 처리를 한다.
결과적으로 mid를 출력하면 된다.
 */
